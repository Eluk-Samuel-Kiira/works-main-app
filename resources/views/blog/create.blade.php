@extends('layouts.app')
@section('title', 'Create Blog Post - Stardena Works')
    ...
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
            ...
            <!-- <button class="btn btn-outline-primary" id="submitDraftBtn" onclick="submitBlogPost('draft')">
                <span id="submitDraftBtnText"><i class="ti ti-device-floppy me-1"></i> Save Draft</span>
                ...
                <div class="col-12">
                    ...
                    <div class="border rounded-3 p-3 text-center bg-light" style="border-style:dashed!important">
                        ...
                        <div id="coverImagePreview" style="display:none; margin-bottom:15px">
                            <img id="coverImagePreviewImg" src="" style="max-width:100%; max-height:200px; border-radius:8px">
                        </div>
                        ...
                        <button type="button" class="btn btn-outline-danger ms-2" onclick="clearCoverImage()" style="display:none" id="clearCoverBtn">
...
// Rich editor sync function
function syncRichEditor(editorId) {
...
function syncAllEditors() {
    syncRichEditor('f_content_editor');
}
...
document.addEventListener('change', function(e) {
    if (e.target && e.target.id === 'f_author_id') {
        const selectedOption = e.target.options[e.target.selectedIndex];
        const authorName = selectedOption.textContent;
        const authorNameField = document.getElementById('f_author_name');
...
async function submitBlogPost(mode = 'live') {
    ...
    // Get content from rich editor hidden field
    ...
    // Handle tags - ensure they are an array
    ...
    try {
        ...
        toast(isDraft ? 'Blog draft saved successfully!' : 'Blog post published successfully!', 'success');
        ...
        if (errorDiv) {
            errorDiv.innerHTML = `
                <div class="alert alert-success">
                    ...
                    <strong>${isDraft ? 'Draft saved!' : 'Blog published!'}</strong>
                    <a href="/blog/${res.data?.slug || ''}" class="alert-link ms-2" target="_blank">
                        View blog <i class="ti ti-external-link ms-1"></i>
                    </a>
                </div>`;
        }
        
        setTimeout(() => {
            if (confirm(isDraft ? 'Draft saved! Would you like to continue editing?' : 'Blog published! Would you like to create another?')) {
                clearForm();
            } else {
        ...
        if (typeof err === 'string') errorMessage = err;
            ...
            errorMessage = 'Failed to publish blog. Please try again.';
        }
...
function clearForm() {
    ...
    // Clear rich editor
    const contentEditor = document.getElementById('f_content_editor');
    ...
    document.getElementById('f_is_published').checked = false;
    document.getElementById('f_sort_order').value = 0;
    document.getElementById('f_robots').value = 'index,follow';
    clearCoverImage();
