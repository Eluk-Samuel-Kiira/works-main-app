<script>
    ...
    const API_BASE   = '/api/v1/job-locations';
    ...
    async function loadItems(page = 1) {
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-3"><i class="ti ti-alert-circle me-1"></i>Failed to load job locations.</td></tr>`;
        }
    }
    ...
    function renderTable(items) {
        ...
        if (!items || items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-muted">No job locations found.</td></tr>`;
            return;
        }
        tbody.innerHTML = items.map((item, i) => `
            <tr>
                <td class="text-muted">${((currentPage - 1) * 15) + i + 1}</td>
                <td><span class="fw-semibold">${esc(item.country)}</span></td>
                ...
                <td>
                    <div class="d-flex align-items-center gap-1">
                        ...
                        <button class="btn btn-sm bg-danger-subtle text-danger p-1" title="Delete" onclick="openDeleteModal(${item.id}, '${esc(item.district)}, ${esc(item.country)}')"><i class="ti ti-trash" style="font-size:1.25rem;"></i></button>
                    </div>
                </td>
            </tr>
        `).join('');
    ...
    async function openViewModal(id) {
        ...
        try {
            ...
            document.getElementById('viewModalTitle').textContent = `${item.district}, ${item.country}`;
            document.getElementById('viewModalBody').innerHTML = `
            <table class="table table-bordered mb-0">
                <tr><th width="180">Country</th><td>${esc(item.country)}</td></tr>
                <tr><th>District</th><td>${esc(item.district)}</td></tr>
    ...
    function openCreateModal() {
        currentId = null;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-plus me-2"></i>Add Location';
    ...
    async function openEditModal(id) {
        currentId = id;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-pencil me-2"></i>Edit Location';
...
<script>
    function openImportModal() {
        ...
        document.getElementById('importFailedList').innerHTML = '';
    ...
    async function submitImport() {
        ...
        const countryOverride = document.getElementById('importCountryOverride').value;
        if (countryOverride) formData.append('country', countryOverride);
        formData.append('skip_duplicates', document.getElementById('importSkipDuplicates').checked ? '1' : '0');
        ...
        try {
            ...
            const { created_count, skipped_count, failed_count, failed } = data.data;
            document.getElementById('importCreatedCount').textContent = created_count;
            document.getElementById('importSkippedCount').textContent = skipped_count;
            document.getElementById('importFailedCount').textContent = failed_count;
            document.getElementById('importFailedList').innerHTML = (failed || [])
                .map(f => `Row ${f.row}: ${esc(f.reason)}`).join('<br>');
            document.getElementById('importResults').classList.remove('d-none');
 
            toast(`Import complete: ${created_count} created, ${skipped_count} skipped, ${failed_count} failed.`,
