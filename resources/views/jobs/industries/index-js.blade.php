<script>
    ...
    // CONFIG & STATE
    // ============================================================
    const API_BASE   = '/api/v1/industries';
    ...
    async function loadItems(page = 1) {
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-3">
                <i class="ti ti-alert-circle me-1"></i>Failed to load industries.</td></tr>`;
        }
    }
    ...
    function renderTable(items) {
        ...
        if (!items || items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-muted">No industries found.</td></tr>`;
            return;
        }
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex align-items-center gap-1">
                        <button class="btn btn-sm bg-primary-subtle text-primary p-1" title="View" onclick="openViewModal(${item.id})">
                            <i class="ti ti-eye" style="font-size:1.25rem;"></i>
                        </button>
                        <button class="btn btn-sm bg-success-subtle text-success p-1" title="Edit" onclick="openEditModal(${item.id})">
                            <i class="ti ti-pencil" style="font-size:1.25rem;"></i>
                        </button>
                        <button class="btn btn-sm bg-danger-subtle text-danger p-1" title="Delete" onclick="openDeleteModal(${item.id}, '${esc(item.name)}')">
    ...
    async function openViewModal(id) {
        ...
        try {
            ...
            document.getElementById('viewModalBody').innerHTML = buildViewHtml(item);
        } catch (e) {
            document.getElementById('viewModalBody').innerHTML =
    ...
    function openCreateModal() {
        currentId = null;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-plus me-2"></i>Add Industry';
    ...
    async function openEditModal(id) {
        currentId = id;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-pencil me-2"></i>Edit Industry';
    ...
    async function submitSave() {
        ...
        try {
            if (currentId) {
                ...
                toast('Industry created successfully.');
            }
            bsModal('formModal').hide();
            loadItems(currentPage);
        } catch (e) {
            const msg = e.message ?? 'Validation failed.';
            const errors = e.errors ? Object.values(e.errors).flat().join('<br>') : '';
            toast(errors || msg, 'error');
        } finally {
            btn.disabled = false;
