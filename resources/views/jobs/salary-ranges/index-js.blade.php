<script>
    ...
    const API_BASE   = '/api/v1/salary-ranges';
    ...
    function formatMoney(val, currency) {
        if (val == null) return '—';
        return Number(val).toLocaleString('en-UG') + (currency ? ` ${currency}` : '');
    }
    ...
    async function loadItems(page = 1) {
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="8" class="text-center text-danger py-3"><i class="ti ti-alert-circle me-1"></i>Failed to load salary ranges.</td></tr>`;
        }
    }
    ...
    function renderTable(items) {
        ...
        if (!items || items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="8" class="text-center py-4 text-muted">No salary ranges found.</td></tr>`;
            return;
        }
        tbody.innerHTML = items.map((item, i) => `
            <tr>
                ...
                <td>${formatMoney(item.min_salary, item.currency)}</td>
                <td>${formatMoney(item.max_salary, item.currency)}</td>
                <td><span class="badge bg-light text-dark border">${esc(item.currency) || '—'}</span></td>
    ...
    async function openViewModal(id) {
        ...
        try {
            ...
            <table class="table table-bordered mb-0">
                ...
                <tr><th>Min Salary</th><td>${formatMoney(item.min_salary, item.currency)}</td></tr>
                <tr><th>Max Salary</th><td>${formatMoney(item.max_salary, item.currency)}</td></tr>
                <tr><th>Currency</th><td>${esc(item.currency) || '—'}</td></tr>
    ...
    function openCreateModal() {
        currentId = null;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-plus me-2"></i>Add Salary Range';
        document.getElementById('formId').value = '';
        document.getElementById('formName').value = '';
        document.getElementById('formCurrency').value = 'UGX';
        document.getElementById('formMinSalary').value = '';
        document.getElementById('formMaxSalary').value = '';
    ...
    async function openEditModal(id) {
        currentId = id;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-pencil me-2"></i>Edit Salary Range';
        ...
        try {
            ...
            document.getElementById('formCurrency').value = item.currency ?? 'UGX';
            document.getElementById('formMinSalary').value = item.min_salary ?? '';
            document.getElementById('formMaxSalary').value = item.max_salary ?? '';
    ...
    async function submitSave() {
        ...
        const minSalary = document.getElementById('formMinSalary').value;
        const maxSalary = document.getElementById('formMaxSalary').value;
        const payload = {
            name: document.getElementById('formName').value.trim(),
            currency: document.getElementById('formCurrency').value || null,
        ...
        try {
            if (currentId) {
                ...
                toast('Salary range updated successfully.');
            } else {
                await apiFetch(API_BASE, { method: 'POST', body: JSON.stringify(payload) });
                toast('Salary range created successfully.');
            }
    ...
    async function confirmDelete() {
        ...
        try {
            ...
            toast('Salary range deleted successfully.');
