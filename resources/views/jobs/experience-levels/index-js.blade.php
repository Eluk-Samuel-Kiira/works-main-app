<script>
    ...
    const API_BASE   = '/api/v1/experience-levels';
    ...
    async function loadItems(page = 1) {
        ...
        try {
            ...
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-danger py-3"><i class="ti ti-alert-circle me-1"></i>Failed to load experience levels.</td></tr>`;
        }
    }
    ...
    function renderTable(items) {
        ...
        if (!items || items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center py-4 text-muted">No experience levels found.</td></tr>`;
            return;
        }
        tbody.innerHTML = items.map((item, i) => `
            <tr>
                ...
                <td><span class="badge bg-light text-dark border">${yearsLabel(item.min_years, item.max_years)}</span></td>
    ...
    async function openViewModal(id) {
        ...
        try {
            ...
            <table class="table table-bordered mb-0">
                ...
                <tr><th>Min Years</th><td>${item.min_years ?? '—'}</td></tr>
                <tr><th>Max Years</th><td>${item.max_years ?? '—'}</td></tr>
                <tr><th>Years Range</th><td>${yearsLabel(item.min_years, item.max_years)}</td></tr>
    ...
    function openCreateModal() {
        currentId = null;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-plus me-2"></i>Add Experience Level';
        document.getElementById('formId').value = '';
        document.getElementById('formName').value = '';
        document.getElementById('formMinYears').value = '';
        document.getElementById('formMaxYears').value = '';
    ...
    async function openEditModal(id) {
        currentId = id;
        document.getElementById('formModalTitle').innerHTML = '<i class="ti ti-pencil me-2"></i>Edit Experience Level';
        ...
        try {
            ...
            document.getElementById('formMinYears').value = item.min_years ?? '';
            document.getElementById('formMaxYears').value = item.max_years ?? '';
    ...
    async function submitSave() {
        ...
        const minYears = document.getElementById('formMinYears').value;
        const maxYears = document.getElementById('formMaxYears').value;
        const payload = {
            name: document.getElementById('formName').value.trim(),
            min_years: minYears !== '' ? parseInt(minYears) : null,
        ...
        try {
            if (currentId) {
                ...
                toast('Experience level updated successfully.');
            } else {
                await apiFetch(API_BASE, { method: 'POST', body: JSON.stringify(payload) });
                toast('Experience level created successfully.');
            }
    ...
    async function confirmDelete() {
        ...
        try {
            ...
            toast('Experience level deleted successfully.');
